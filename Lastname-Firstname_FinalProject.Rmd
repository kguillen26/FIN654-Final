...
4.	The company wants to mitigate their risk by using future contracts, which will require closely monitoring the agricultural spot market and continued speculation on future price movements.
...
- The scatterplot illustrates the outliers of each of the three ingrediants.  Wheat appears to be more concentrated around the mean but also experienced its share of volatility.  This has a direct impact on risk measures associated to portfolio performance.
...
- There is a strong correlation (.89) between Corn and Soybean.  This was initially expected since both of these ingredients are used in similar applications or in the creation of bread and other foods. Both are a significant raw product for a number of base foods.  
...
* `r buy_sell_corn` `r dollar(weights[ind,1]*money_to_spend)` in Corn
* `r buy_sell_soybean` `r dollar(weights[ind,2]*money_to_spend)` in Soybean
* `r buy_sell_wheat` `r dollar(weights[ind,3]*money_to_spend)` in Wheat
...
* `r spend_format` denominated in dollars
* `r buy_sell_corn2` `r dollar(weights[ind2,1]*money_to_spend)` in Corn
* `r buy_sell_soybean2` `r dollar(weights[ind2,2]*money_to_spend)` in Soybean
* `r buy_sell_wheat2` `r dollar(weights[ind2,3]*money_to_spend)` in Wheat
...
* `r spend_format` denominated in dollars
* `r buy_sell_corn3` `r dollar(weights.x[inx,1]*money_to_spend)` in Corn
* `r buy_sell_soybean3` `r dollar(weights.x[inx,2]*money_to_spend)` in Soybean
* `r buy_sell_wheat3` `r dollar(weights.x[inx,3]*money_to_spend)` in Wheat
...
- The working capital account of \$250 million euro should be allocated as follows: buy \$260,880,889 million in Corn, \$20,553,257 million in Soybean, sell \$31,434,147 million in Wheat.
